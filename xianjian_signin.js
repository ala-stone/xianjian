...
async function signInAccount(account) {
    ...
    // 获取user_id（从登录响应中获取，或使用配置中的）
    const userId = loginResult.user_id || account.user_id;
    if (!userId) {
        log(`  → 警告: 未获取到user_id，尝试使用配置中的值`);
    } else {
        log(`  → 获取user_id: ${userId}`);
    }
    ...
    // 3. 执行签到
    const signInResult = await doSignIn(account, cookie, roleInfo, userId);
    ...
    try {
        ...
        if (body.code === 200 && body.data) {
            return {
                success: true,
                cookie: cookie,
                user_id: body.data.user_id || null,  // 提取user_id
                message: "登录成功"
            };
        } else {
            return {
 ...
 * @returns {Promise<Object>} - 签到结果
 */
async function doSignIn(account, cookie, roleInfo, userId) {
    // 构建签到参数
